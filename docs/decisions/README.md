---
id: ADR-INDEX
artifact: decision-index
status: current
last_reviewed: 2026-09-17
---

# Architecture Decision Records

The split ADRs below are the AI-friendly decision index. The removed chronological predecessor and its semantic reconciliation are recorded in the [documentation reconciliation](../implementation/documentation-reconciliation.md).

| ID | Decision | Outcome |
|---|---|---|
| [ADR-001](ADR-001-draft-lifetime-deferred.md) | Draft lifetime across UC-01/UC-02 requests | Superseded by ADR-016 |
| [ADR-002](ADR-002-workload-output-ordering.md) | Dependency-ordered output propagation | Accepted |
| [ADR-003](ADR-003-resource-instance-ownership.md) | Resource Instance ownership and reuse | Accepted |
| [ADR-004](ADR-004-uc04-provider-preconditions-deferred.md) | UC-04 provider preconditions | Deferred |
| [ADR-005](ADR-005-immutable-application-versions.md) | Immutable Application Definition versions | Accepted |
| [ADR-006](ADR-006-typed-plan-deferred.md) | Typed Infrastructure Plan | Deferred |
| [ADR-007](ADR-007-progress-ownership-deferred.md) | Progress-marker ownership | Deferred |
| [ADR-008](ADR-008-lifecycle-delivery-status-deferred.md) | Separate lifecycle and delivery status | Deferred |
| [ADR-009](ADR-009-canonical-enums.md) | Canonical ENUM catalog | Accepted |
| [ADR-010](ADR-010-background-deployment-worker.md) | Background Deployment Worker | Accepted |
| [ADR-011](ADR-011-orphaned-secret-deferred.md) | Orphaned Secret compensation | Deferred |
| [ADR-012](ADR-012-target-and-catalog-versioning.md) | Deployment target and Catalog Version | Accepted |
| [ADR-013](ADR-013-per-application-delivery-repository.md) | Per-application Delivery Repository | Accepted |
| [ADR-014](ADR-014-fleet-default-cd-provider.md) | Fleet as default CD provider | Accepted |
| [ADR-015](ADR-015-separate-remove-use-case.md) | Separate UC-05 removal use case | Accepted |
| [ADR-016](ADR-016-client-owned-drafts.md) | Browser-owned UC-01/UC-02 drafts with optimistic concurrency | Accepted |

An ADR with a deferred outcome records that deferral was accepted; the unresolved technical work remains canonical in the corresponding backlog item.
