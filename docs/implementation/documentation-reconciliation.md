...
This record is the manual semantic gate for consolidated predecessors removed from the working tree after reconciliation. Structural checks cannot prove that a split artifact retains every current concept, so each source group was read in full and mapped below before removal. The exact predecessors remain available from commit `f58765e` with the `git show` commands recorded below.

## Use-case specification and realization source

Source: `git show f58765e:docs/archive/consolidated/usecase-realization-step-1-3.md`.
...
| Introductory explanation of the three-step realization process | Git history | `HISTORICAL` — process provenance, not a current product definition |

## Decision log

Source: `git show f58765e:docs/archive/consolidated/design-decisions-log.md`.
...
| Overview table and branch/editing chronology | Git history | `HISTORICAL` — summarized by the ADR index and retained only as chronology |

## Deferred-issue log

Source: `git show f58765e:docs/archive/consolidated/deferred-issues-log.md`.
...
| Source-branch provenance | Git history | `HISTORICAL` — not a current design definition |

## Consolidated verification log

Source: `git show f58765e:docs/archive/consolidated/uc03-verification-log.md`.
...
Source: `git show f58765e:docs/archive/planning/uc03-original-implementation-plan.md`.

The entire plan was reviewed, including §10, §10.1, the complete A1 list, later progress notes, and the §12 decision dialogue. Statements that describe an earlier checkout remain in Git history; current behavior is owned by the artifacts below.

| Plan area | Current owner or classification | Result |
|---|---|---|
| §1–§4 original context, assumptions, and proposed package layout | Git history; current navigation is the [implementation index](README.md) and [`idp/backend/README.md`](../../idp/backend/README.md) | `HISTORICAL` |
...
The audited predecessors contain no current concept whose only owner is in Git history. Current definitions live in use-case, architecture, implementation, decision, backlog, operations, or evidence artifacts; the deleted sources remain non-normative provenance.
