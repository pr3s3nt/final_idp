...
- **Cross References**: UC-02 – Configure Application Environment, luồng chính, A1 – Configuration không hợp lệ và A2 – Draft đã cũ; [ADR-020](../../decisions/ADR-020-uc02-catalog-version-and-target.md).
- **Preconditions**:
  ...
  - `configurationDraft.catalogVersion` là một `Catalog Version` đang tồn tại và `configurationDraft.deploymentTarget` là một deployment target được hỗ trợ. Mỗi `Resource Requirement` của phiên bản mới nhất resolve được về **đúng một** `Resource Definition` theo cặp này, dùng cùng quy tắc UC-03 dùng: khớp `resource_type`, `supported_contexts` chứa deployment context của target, và `applicability_conditions` khớp application + environment.
  ...
  - `Direct Configuration Value` chỉ được dùng cho Environment Variable thông thường. Mỗi `Resource Output Reference` tham chiếu một `Resource Requirement` trong phiên bản mới nhất và một output có trong `exposedOutputs` của `Resource Definition` được resolve từ `catalogVersion` + `deploymentTarget` của draft; mỗi `Workload Output Reference` tham chiếu một Workload trong phiên bản mới nhất và một output có trong `Workload.exposedOutputs`.
  ...
  - Mỗi `Secret` tham chiếu đúng `Secret Definition` và Workload sở hữu definition đó, đồng thời có đúng một source: opaque `secretReference` do Secret Store trả về hoặc sensitive `Resource Output Reference` có output nằm trong `sensitiveOutputs` của cùng `Resource Definition` được resolve ở trên.
...
- **Postconditions**:
  ...
  - `catalogVersion` và `deploymentTarget` của draft không được persist: không cột nào của `environment_configuration` giữ chúng, và configuration vẫn không gắn với phiên bản catalog hay deployment target nào. Contract 6 của UC-03 vẫn kiểm tra lại configuration theo phiên bản catalog của lần deploy.
- **Exceptions / Guarantees**:
  - A1: nếu thiếu giá trị bắt buộc, resource/workload không tồn tại trong phiên bản mới nhất, output không được expose bởi `Resource Definition` được resolve, output thuộc thành phần mà workload sở hữu biến không depends on, hoặc một `Resource Requirement` không resolve được về đúng một `Resource Definition` theo `catalogVersion` + `deploymentTarget`, toàn bộ state trong `environment_configuration`, `environment_variable`, `configuration_value` và `secret` giữ nguyên như trước operation.
...
- **Postconditions**:
  - Effective override lấy `resource_instance.applied_overrides` của lần apply gần nhất làm baseline rồi merge override hợp lệ Developer gửi khi confirm. Không gửi lại một key không có nghĩa là xóa baseline của key đó.
  - Với mỗi item create (`MANAGED`) thành công, provisioner được gọi với tham số đã resolve và `requiredResourceOutputs`; một instance `Resource Instance` và một row `resource_instance` được tạo với `application_id`, `environment`, `resource_requirement_id`, `resource_definition_id` (definition của phiên bản catalog), `deployment_target`, `infrastructure_reference`, `provider_state_reference`, `applied_overrides`, `applied_input_fingerprint` (hash của definition, tham số đã resolve và dấu vân tay output của các Platform Requirement được `requires`), `created_at`, `updated_at` và `status = READY`.
  - Với mỗi item update (`MANAGED`) thành công, provisioner được gọi với input hiện tại; `resource_definition_id`, `infrastructure_reference`, `provider_state_reference`, `applied_overrides`, `applied_input_fingerprint`, `status = READY`, `updated_at` được cập nhật theo lần apply này; identity và `created_at` không đổi.
  - Với mỗi item reuse, instance hiện hữu được giữ và provisioner không được gọi; identity, `infrastructure_reference`, `applied_overrides` và `applied_input_fingerprint` không đổi, `status` được xác nhận là `READY`. Nếu Deployment dùng phiên bản catalog khác nhưng đầu vào không đổi, `resource_definition_id` được trỏ sang definition cùng `name` của phiên bản catalog đang dùng.
...
- **Postconditions**:
  ...
  - Tiến trình theo tầng/thành phần được thể hiện qua các row `deployment_step` (có `wave_number`, `related_component_reference`). Implementation hiện tạo step khi bắt đầu chạy; D4 vẫn giữ câu hỏi thiết kế tổng quát về ownership và mô hình progress lâu dài.
...
- **Exceptions / Guarantees**:
  ...
  - Việc upsert `deployment_record`, đổi `deployment.status`, chuyển execution job sang `COMPLETED`/`FAILED`, chuyển step còn `PENDING`/`RUNNING` sang `SKIPPED`, và ghi `deployment_record_resource_instance` là một transaction: nếu repository write thất bại, không để lại record/status/job/association chỉ được persist một phần.
