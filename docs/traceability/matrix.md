...
|---|---|---|---|---|---|---|---|
| UC-01 | Luồng chính: Developer chọn **Create Application** | `createApplication()` | `docs/use-cases/UC-01/sequence.puml`: Web UI self-message, tạo `ApplicationDefinitionDraft` | Web UI | — *(CLIENT-OWNED DTO trong browser `sessionStorage`)* | Không | Không |
| UC-01 | Luồng chính: Developer mở application hiện có để chỉnh sửa | `updateApplication()` | `docs/use-cases/UC-01/sequence.puml`: Application API / Controller → Application Service; trả DTO có `baseVersion` cho Web UI | Application API / Controller → Application Service; Web UI sở hữu DTO sau response | Đọc `application_definition`, `application_definition_version`, `workload`, `resource_requirement`, `environment_variable_definition`, `secret_definition`, `dependency` của phiên bản mới nhất; không ghi | Không | Không |
| UC-01 | Luồng chính: **Add Resource** và khai báo name/type | `addResourceRequirement()` | `docs/use-cases/UC-01/sequence.puml`: Web UI self-message | Web UI | — *(CLIENT-OWNED DTO trong browser `sessionStorage`)* | Không | Không |
| UC-01 | Luồng chính: **Add Workload** và khai báo type/image repository/port | `addWorkload()` | `docs/use-cases/UC-01/sequence.puml`: Web UI self-message | Web UI | — *(CLIENT-OWNED DTO trong browser `sessionStorage`)* | Không | Không |
| UC-01 | Luồng chính: khai báo Environment Variable và Secret cho workload | `defineConfigurationRequirement()` | `docs/use-cases/UC-01/sequence.puml`: Web UI self-message | Web UI | — *(CLIENT-OWNED DTO trong browser `sessionStorage`)* | Không | Không |
| UC-01 | Luồng chính: khai báo quan hệ **depends on** | `defineDependency()` | `docs/use-cases/UC-01/sequence.puml`: Web UI self-message | Web UI | — *(CLIENT-OWNED DTO trong browser `sessionStorage`)* | Không | Không |
| UC-01 | Luồng chính/A1: IDP kiểm tra Application Definition | `validateApplicationDefinition()` | `docs/use-cases/UC-01/sequence.puml`: Application Definition Validator | Application Definition Validator | — *(validate object graph trong memory)* | Không | Không |
| UC-01 | Luồng chính/A2: Developer chọn **Save Application**, IDP kiểm tra `baseVersion` và lưu phiên bản mới | `saveApplicationDefinition()` | `docs/use-cases/UC-01/sequence.puml`: Application API / Controller → Application Service; internal repository message `saveNewVersionIfBaseMatches()` thực hiện compare-and-write atomically | Application API / Controller → Application Service | Trong một transaction/CAS, đọc latest `application_definition_version`; khi base match, insert `application_definition_version`, `application_component` (thành phần mới), `workload`, `resource_requirement`, `environment_variable_definition`, `secret_definition`, `dependency` theo phiên bản và create/update `application_definition`; conflict không ghi | **Có — Contract 1** | Không |
| UC-01 | Luồng chính: IDP sinh application specification cho phiên bản mới | `generateApplicationSpecification()` | `docs/use-cases/UC-01/sequence.puml`: Application Specification Generator | Application Specification Generator | Ghi `application_specification` (`UNIQUE application_definition_version_id`) | **Có — Contract 2** | Không |
| UC-02 | Luồng chính: Developer chọn environment cần cấu hình | `selectEnvironment()` | `docs/use-cases/UC-02/sequence.puml`: Environment Configuration API / Controller → Environment Configuration Service; trả client-owned DTO cùng hai concurrency base, danh sách Catalog Version và deployment target | Environment Configuration API / Controller → Environment Configuration Service; Web UI sở hữu DTO sau response | Đọc `environment_configuration` hiện hành/revision, dữ liệu requirement và `catalog_version`; không ghi | Không | Không |
| UC-02 | Luồng chính: IDP hiển thị Variable/Secret đã khai báo trong UC-01 | `loadConfigurationRequirements()` | `docs/use-cases/UC-02/sequence.puml`: Application Query / Application Repository | Application Query / Application Repository | Đọc `application_definition`, `application_definition_version` (phiên bản mới nhất), `workload`, `environment_variable_definition`, `secret_definition`, `dependency` | Không | Không |
| UC-02 | Luồng chính: Developer nhập direct value cho Variable hoặc Secret | `setDirectConfigurationValue()` | `docs/use-cases/UC-02/sequence.puml`: Environment Variable là Web UI self-message; Secret đi qua Environment Configuration API / Controller → Service → Secret Store và UI chỉ nhận opaque reference | Web UI; riêng secure Secret staging qua Environment Configuration API / Controller → Environment Configuration Service | — *(CLIENT-OWNED DTO; plaintext Secret không vào `sessionStorage`; Secret Store giữ value, D07 quản lý lifecycle)* | Không | Không |
| UC-02 | Luồng chính: Developer chọn Catalog Version và deployment target | `selectCatalogVersionAndTarget()` | `docs/use-cases/UC-02/sequence.puml`: Web UI self-message sau khi nhận danh sách | Web UI | — *(CLIENT-OWNED DTO; ADR-020 giữ hai giá trị ngoài durable storage)* | Không | Không |
| UC-02 | Luồng chính: Developer chọn Resource Output / sensitive Resource Output | `bindResourceOutput()` | `docs/use-cases/UC-02/sequence.puml`: Web UI self-message sau catalog query | Web UI | — *(CLIENT-OWNED DTO; output hợp lệ là `exposed_outputs`/`sensitive_outputs` của `resource_definition` được resolve từ catalogVersion + deploymentTarget)* | Không | Không |
| UC-02 | Luồng chính: Developer chọn Workload Output | `bindWorkloadOutput()` | `docs/use-cases/UC-02/sequence.puml`: Web UI self-message sau catalog query | Web UI | — *(CLIENT-OWNED DTO; output hợp lệ lấy từ `workload.exposed_outputs`)* | Không | Không |
| UC-02 | Luồng chính/A1: IDP kiểm tra value và output references | `validateEnvironmentConfiguration()` | `docs/use-cases/UC-02/sequence.puml`: Environment Configuration Validator | Environment Configuration Validator | — *(validate configuration draft và catalog results trong memory theo phiên bản mới nhất; output phải thuộc thành phần mà workload depends on và phải có trong Resource Definition được resolve)* | Không | Không |
| UC-02 | Luồng chính/A2: Developer chọn **Save Configuration**, IDP kiểm tra hai concurrency base và lưu theo environment | `saveEnvironmentConfiguration()` | `docs/use-cases/UC-02/sequence.puml`: Environment Configuration API / Controller → Environment Configuration Service; internal repository message `saveIfBasesMatch()` thực hiện compare-and-write atomically | Environment Configuration API / Controller → Environment Configuration Service | Trong một transaction/CAS, đọc latest `application_definition_version` và `environment_configuration.updated_at`/opaque revision; khi base match, ghi `environment_configuration`, `environment_variable`, `configuration_value`, `secret`; conflict không ghi | **Có — Contract 3** | Không |
| UC-03 | Luồng chính: chọn phiên bản, phiên bản catalog, nơi triển khai, workload, image/context và tạo deployment để lập plan | `createDeployment()` | `docs/use-cases/UC-03/sequence.puml`: Deployment API / Controller → Deployment Orchestrator; Orchestrator canonicalize/hash plan; internal repository message `persistDeployment(...)` ở `AWAITING_CONFIRMATION` | Deployment API / Controller → Deployment Orchestrator | Đọc `application_definition_version` và các bảng theo phiên bản, `environment_configuration`, `catalog_version`, `resource_definition` của phiên bản catalog (gồm `default_parameters`, `allowed_overrides`, `management_mode`, `requires`), `resource_instance`, `workload_instance`; ghi `deployment` (gồm `application_definition_version_id`, `catalog_version_id`, `plan_fingerprint`, `plan_fingerprint_algo`), `workload_deployment` (`SELECTED`, `wave_number`), `deployment_context` với status `AWAITING_CONFIRMATION`; plan vẫn transient | **Có — Contract 4** | **Có — Deployment:** execution-only Created/Validating → `AWAITING_CONFIRMATION`, hoặc Rejected/A1 |
| UC-03 | Luồng chính/A1: IDP kiểm tra image, context, configuration khớp phiên bản và luật deploy một phần | `validateDeploymentInput()` | `docs/use-cases/UC-03/sequence.puml`: Deployment Orchestrator tự nhận message | Deployment Orchestrator | Đọc state đã load từ các bảng theo phiên bản, `environment_configuration`, `environment_variable`, `configuration_value`, `secret`, `workload_instance` (phiên bản đang chạy) | Không *(nằm trong Contract 4)* | **Có — Deployment:** guard valid/invalid từ Created/Validating |
| UC-03 | Luồng chính: dựng dependency/resource graph, tự thêm cụm Kubernetes và network | `buildDeploymentGraph()` | `docs/use-cases/UC-03/sequence.puml`: Deployment Graph Builder (Orchestrator và Deployment Worker cùng gọi); Graph Builder gọi `resolveResourceDefinitions()` lặp tới khi không còn Platform Requirement mới | Deployment Graph Builder | Đọc dữ liệu đã load từ `workload`, `resource_requirement`, `dependency` của phiên bản, `environment_configuration`, `configuration_value`, `secret`, cùng `resource_definition.requires` qua Resolver; `Deployment Graph` là transient, không có table; ID Platform Requirement suy ra từ application + loại, chưa ghi `application_component` | Không | **Có — Deployment:** action trước khi vào `AWAITING_CONFIRMATION` |
| UC-03 | Luồng chính: xác định phạm vi (kể cả cụm/network mà resource cần), chia tầng, danh sách có thể bị làm lại | `planDeploymentWaves()` | `docs/use-cases/UC-03/sequence.puml`: Deployment Wave Planner (Orchestrator và Deployment Worker cùng gọi); sau khi có plan, internal message `findPotentialRedeploys()` | Deployment Wave Planner | Đọc `workload_instance` (workload phụ thuộc đang chạy); kết quả transient trong Deployment Graph | Không *(nằm trong Contract 4)* | **Có — Deployment:** action trước khi vào `AWAITING_CONFIRMATION` |
| UC-03 | Luồng chính: resolve Resource Definition trong phiên bản catalog cho resource, cụm Kubernetes và network | `resolveResourceDefinitions()` | `docs/use-cases/UC-03/sequence.puml`: Resource Definition Resolver (do Deployment Graph Builder gọi); internal repository message `findResourceDefinitions(catalogVersion)` tới Resource Definition Catalog | Resource Definition Resolver | Đọc `resource_requirement`, `catalog_version`, `resource_definition` của phiên bản (gồm `management_mode`, `applicability_conditions`, `requires`) | Không | **Có — Deployment:** action trước khi vào `AWAITING_CONFIRMATION` |
| UC-03 | Luồng chính: xác định create/update/reuse/liên kết infrastructure và thành phần cần gỡ | `planInfrastructureChanges()` | `docs/use-cases/UC-03/sequence.puml`: Infrastructure Planner; chạy khi create, khi confirm trước khi tính lại fingerprint, và trong Deployment Worker | Infrastructure Planner | Đọc `resource_instance` theo khóa chủ sở hữu (gồm `applied_input_fingerprint`), `workload_instance`; plan là transient, chỉ fingerprint + algorithm được persist trên `deployment` | Không | **Có — Deployment:** action trước `AWAITING_CONFIRMATION` và guard rebuild khi confirm; **Resource Instance:** vào phase Planned (`PLANNED`) cho create/update `MANAGED` |
| UC-03 | Luồng chính: hiển thị parameter Developer được phép override | `loadInfrastructureOverrides()` | `docs/use-cases/UC-03/sequence.puml`: Infrastructure Planner | Infrastructure Planner | Đọc policy từ `resource_definition.allowed_overrides`; danh sách áp dụng nằm trong transient plan | Không | Không |
| UC-03 | Luồng chính: Developer đặt permitted overrides | `applyInfrastructureOverrides()` | `docs/use-cases/UC-03/sequence.puml`: Deployment Orchestrator → Infrastructure Planner (validate khi confirm); Deployment Worker → Infrastructure Planner (áp override từ job) | Deployment Orchestrator / Deployment Worker → Infrastructure Planner | — *(final plan transient; override đã chọn lưu ở `deployment_execution_job.selected_overrides`)* | Không *(postcondition được gộp trong Contract 5)* | **Có — Deployment:** action của transition xác nhận hợp lệ |
| UC-03 | Luồng chính/A1: Developer xác nhận và chọn **Deploy** | `confirmDeployment()` | `docs/use-cases/UC-03/sequence.puml`: Deployment API / Controller → Deployment Orchestrator; load persisted inputs, rebuild plan, recompute/compare fingerprint, validate overrides, rồi internal repository message `confirmDeploymentAndCreateJob(...)` trong một transaction; trả lời ngay | Deployment API / Controller → Deployment Orchestrator | Đọc `deployment`, `workload_deployment`, `deployment_context`, `application_definition_version` và các bảng theo phiên bản, `environment_configuration`, `catalog_version`, `resource_definition` của phiên bản catalog, `resource_instance`, `workload_instance`; có thể refresh fingerprint khi `PLAN_CHANGED`; atomic CAS `deployment.status = CONFIRMED` và insert `deployment_execution_job` cùng transaction | **Có — Contract 5** | **Có — Deployment:** self-transition `PLAN_CHANGED`/invalid override; `AWAITING_CONFIRMATION` → `CONFIRMED` (có job) chỉ khi fingerprint match và CAS thành công; CAS loser không transition, không job |
| UC-03 | Luồng chính/A2: Deployment Worker reconcile infrastructure của một tầng (kể cả cụm Kubernetes, network), hoặc hủy/gỡ liên kết resource không còn trong phiên bản | `reconcileInfrastructure()` | `docs/use-cases/UC-03/sequence.puml`: Infrastructure Reconciler (do Deployment Worker gọi); trước các tầng, Deployment Worker gọi internal repository message `ensurePlatformRequirements()` | Infrastructure Reconciler | Ghi/đọc `resource_instance` theo khóa chủ sở hữu (gồm `resource_definition_id`, `applied_input_fingerprint`); `application_component` loại `PLATFORM_REQUIREMENT` được tạo bởi `ensurePlatformRequirements()`; `deployment.status` giữ `DEPLOYING` | **Có — Contract 6** | **Có — Deployment:** self-transition trong `DEPLOYING` hoặc → `FAILED`; **Resource Instance:** `PLANNED` → `PROVISIONING` → `READY`/`FAILED`, reuse tại `READY`, liên kết `EXISTING` (resource dùng chung, cụm nội bộ) → `READY`, `DESTROYED`, `UNLINKED` |
| UC-03 | Luồng chính: thu thập Resource Output khi infrastructure ready | `collectResourceOutputs()` | `docs/use-cases/UC-03/sequence.puml`: Resource Output Resolver / Collector (do Deployment Worker gọi) | Resource Output Resolver / Collector | Đọc `resource_instance`; không có table `resource_output` vì object transient | Không | **Có — Deployment:** action trong `DEPLOYING`; **Resource Instance:** self-transition tại `READY` |
| UC-03 | Luồng chính/A1/A2: resolve direct value, Resource/Workload Output cho các workload của một tầng | `resolveEnvironmentConfiguration()` | `docs/use-cases/UC-03/sequence.puml`: Environment Configuration Resolver (do Deployment Worker gọi) | Environment Configuration Resolver | Đọc `environment_configuration`, `environment_variable`, `configuration_value`, `secret`, `resource_instance`; `deployment.status` giữ `DEPLOYING` | **Có — Contract 7** | **Có — Deployment:** self-transition trong `DEPLOYING` hoặc → `FAILED` |
| UC-03 | Luồng chính: tạo resolved application specification | `generateResolvedApplicationSpecification()` | `docs/use-cases/UC-03/sequence.puml`: Resolved Specification Generator | Resolved Specification Generator | Đọc snapshot từ `workload_deployment`; `Resolved Specification` là transient, không có table | Không | **Có — Deployment:** action trong `DEPLOYING`; không có state riêng |
| UC-03 | Luồng chính/A2: sinh base Kubernetes manifest bằng score-k8s | `generateKubernetesManifest()` | `docs/use-cases/UC-03/sequence.puml`: Manifest Generator / Score Renderer (do Deployment Worker gọi) | Manifest Generator / Score Renderer | — *(manifest và resolved specification transient)* | Không | **Có — Deployment:** action/failure point trong `DEPLOYING` |
| UC-03 | Luồng chính/A2: áp dụng target-specific adaptation/patch | `adaptManifestForTarget()` | `docs/use-cases/UC-03/sequence.puml`: Target Manifest Adapter (do Deployment Worker gọi) | Target Manifest Adapter | — *(manifest transient)* | Không | **Có — Deployment:** action/failure point trong `DEPLOYING` |
| UC-03 | Luồng chính/A2: materialize Environment Variable | `materializeEnvironmentConfiguration()` | `docs/use-cases/UC-03/sequence.puml`: Environment Configuration Materializer (do Deployment Worker gọi) | Environment Configuration Materializer | — *(desired state transient; không persist resolved value)* | Không | **Có — Deployment:** action/failure point trong `DEPLOYING` |
| UC-03 | Luồng chính/A2: materialize Secret/secret reference | `materializeSecretConfiguration()` | `docs/use-cases/UC-03/sequence.puml`: Secret Materializer (do Deployment Worker gọi) | Secret Materializer | — *(desired state transient; không persist plaintext Secret)* | Không | **Có — Deployment:** action/failure point trong `DEPLOYING` |
| UC-03 | Luồng chính/A2: bảo đảm Delivery Repository của application, publish desired deployment state của một tầng sang CD abstraction; lần publish cuối gỡ workload không còn trong phiên bản | `publishDesiredDeploymentState()` | `docs/use-cases/UC-03/sequence.puml`: CD Integration / CD Provider Interface (do Deployment Worker gọi); internal message `findDeliveryRepository()`, `ensureDeliveryRepository()`, `saveDeliveryRepository()`, `saveWorkloadInstances(...)`/`updateWorkloadInstances(...)` | CD Integration / CD Provider Interface | Đọc/ghi `delivery_repository` (tạo ở lần deploy đầu của application); ghi `workload_instance` (`DEPLOYING`, `current_workload_deployment_id`; `REMOVED` khi gỡ); `deployment.status` giữ `DEPLOYING`; `delivery_reference` chỉ được persist bởi `saveDeploymentRecord()` | **Có — Contract 8** | **Có — Deployment:** action/failure point trong `DEPLOYING`; **Workload Instance:** → `DEPLOYING`, → `REMOVED` |
| UC-03 | Luồng chính/A2: chờ pod của các workload trong tầng healthy | `waitForWorkloadsHealthy()` | `docs/use-cases/UC-03/sequence.puml`: Workload Status Provider / Kubernetes Adapter (do Deployment Worker gọi) | Workload Status Provider / Kubernetes Adapter | — *(đọc health từ Kubernetes Cluster)* | Không *(precondition của Contract 10)* | **Có — Deployment:** failure point trong `DEPLOYING`; **Workload Instance:** `DEPLOYING` → `HEALTHY`/`FAILED` |
| UC-03 | Luồng chính/A2: thu thập Workload Output của workload vừa healthy hoặc workload phụ thuộc đang chạy | `collectWorkloadOutputs()` | `docs/use-cases/UC-03/sequence.puml`: Workload Output Collector (do Deployment Worker gọi); internal message `readWorkloadOutputs()` tới Workload Status Provider | Workload Output Collector | Đọc `workload_instance`; không có table `workload_output` vì object transient | **Có — Contract 10** | **Có — Deployment:** action/failure point trong `DEPLOYING` |
| UC-03 | Luồng chính: so sánh dấu vân tay output, reconcile lại resource phụ thuộc và thêm workload phụ thuộc vào các tầng sau | `propagateOutputChanges()` | `docs/use-cases/UC-03/sequence.puml`: Deployment Wave Planner (do Deployment Worker gọi); internal repository message `loadOutputFingerprints()`, `loadOutputFingerprintsAndRunningImages()`, `updateOutputFingerprints()`, `updateWorkloadInstances()` | Deployment Wave Planner | Đọc/ghi `resource_instance.output_fingerprint`, `workload_instance.output_fingerprint`; đưa resource `MANAGED` `requires` thành phần đổi output vào các tầng sau (update); insert `workload_deployment` (`CASCADED`) khi output đổi | **Có — Contract 11** | **Có — Deployment:** self-transition trong `DEPLOYING`; **Resource Instance:** self-transition tại `READY`, hoặc `READY` → `PLANNED` khi Platform Requirement mà nó `requires` đổi output; **Workload Instance:** → `HEALTHY` |
| UC-03 | Luồng chính/A2: lưu Deployment Record, image, infra reference, thành phần đã gỡ, kết quả | `saveDeploymentRecord()` | `docs/use-cases/UC-03/sequence.puml`: Deployment Repository (do Deployment Worker gọi), sau đó `updateDeploymentStatus(deploymentId, DEPLOYING, SUCCEEDED)` | Deployment Repository | Ghi/cập nhật `deployment_record` (gồm `removed_components`), `deployment_record_resource_instance`, `deployment.status`; actual images đọc từ `workload_deployment`; writer của `deployment_step` thuộc D4 | **Có — Contract 9** | **Có — Deployment:** `DEPLOYING` → `SUCCEEDED` hoặc `FAILED` |
| UC-04 | Luồng chính: mở Deployments và xem lịch sử | `listDeployments()` | `docs/use-cases/UC-04/sequence.puml`: Deployment Query API / Controller → Deployment Query Service | Deployment Query API / Controller → Deployment Query Service | Đọc `deployment`, `deployment_record` | Không | Không — read-only, chỉ quan sát state |
| UC-04 | Luồng chính: chọn một deployment và lấy detail | `getDeploymentDetail()` | `docs/use-cases/UC-04/sequence.puml`: Deployment Query API / Controller → Deployment Query Service → Deployment Repository | Deployment Query API / Controller → Deployment Query Service → Deployment Repository | Đọc `deployment` (gồm `application_definition_version_id`, `catalog_version_id`), `deployment_record`, `deployment_record_resource_instance`; internal message `findWorkloadInstances()` đọc `workload_instance` để biết deployment còn là bản đang chạy hay không | Không | Không — read-only |
| UC-04 | Luồng chính: hiển thị deployment progress theo từng tầng và thành phần | `getDeploymentProgress()` | `docs/use-cases/UC-04/sequence.puml`: Deployment Repository | Deployment Repository | Đọc `deployment_step` (gồm `wave_number`, `related_component_reference`); writer của step thuộc D4 | Không | Không — đọc progress marker, không đổi Deployment state |
| UC-04 | Luồng chính: hiển thị infrastructure status | `getInfrastructureStatus()` | `docs/use-cases/UC-04/sequence.puml`: Resource Instance Repository | Resource Instance Repository | Đọc `deployment_record_resource_instance`, `resource_instance` | Không | Không — đọc Resource Instance state |
| UC-04 | Luồng chính: hiển thị CD status | `getCDStatus()` | `docs/use-cases/UC-04/sequence.puml`: CD Integration / CD Status Provider | CD Integration / CD Status Provider | `deployment_record.delivery_reference` là input đã load; status hiện tại lấy từ external CD System | Không | Không — read-only external status |
| UC-04 | Luồng chính/A2: hiển thị workload health | `getWorkloadStatus()` | `docs/use-cases/UC-04/sequence.puml`: Workload Status Provider / Kubernetes Adapter | Workload Status Provider / Kubernetes Adapter | `deployment.deployment_target` và `workload_deployment` là input đã load; health lấy từ Kubernetes Cluster | Không | Không — read-only runtime status |
| UC-04 | Luồng chính: hiển thị exposed endpoint nếu có | `getDeploymentEndpoints()` | `docs/use-cases/UC-04/sequence.puml`: Workload Status Provider / Kubernetes Adapter | Workload Status Provider / Kubernetes Adapter | `deployment.deployment_target` và `workload_deployment` là input đã load; endpoint lấy từ Kubernetes Cluster | Không | Không — read-only runtime data |
| UC-04 | Luồng chính: hiển thị image repository/version thực tế | `getDeploymentImages()` | `docs/use-cases/UC-04/sequence.puml`: Deployment Repository | Deployment Repository | Đọc `workload_deployment` (gồm `inclusion_reason`, `wave_number`) qua `deployment`/`deployment_record`, cùng `deployment_record.removed_components` | Không | Không — read-only |
| UC-04 | Luồng ngoại lệ A1: hiển thị failed step, component và error summary | `getDeploymentFailureDetail()` | `docs/use-cases/UC-04/sequence.puml`: Deployment Query API / Controller → Deployment Query Service → Deployment Repository trong nhánh deployment failed (A1) | Deployment Query API / Controller → Deployment Query Service → Deployment Repository | Đọc `deployment_record.error_summary` và các row `deployment_step` failed: `status`, `related_component_reference`, `error_summary` | Không | Không — read-only, chỉ quan sát `FAILED` |
| UC-05 | Luồng chính: Developer chọn environment và nơi triển khai cần gỡ; IDP lập plan gỡ bỏ theo thứ tự ngược và hiển thị cảnh báo mất dữ liệu | `createTeardown()` | `docs/use-cases/UC-05/sequence.puml`: Deployment API / Controller → Deployment Orchestrator | Deployment API / Controller → Deployment Orchestrator | Đọc `deployment` (lần gần nhất của cùng chủ sở hữu), `application_definition_version`, `environment_configuration`, `catalog_version`, `resource_definition`, `resource_instance`, `workload_instance`; ghi `deployment` với `kind = TEARDOWN`, `status = AWAITING_CONFIRMATION`; không ghi `workload_deployment` | **Có — Contract 12** | **Có** — entry mới vào state machine Deployment |
...
| Environment Configuration API / Controller | Nhận `selectEnvironment()`, catalog queries, secure Secret staging và complete-draft `saveEnvironmentConfiguration()`; normal field/binding edit chạy ở Web UI | PASS |
...
| Application Service | Stateless giữa edit request; xử lý load và complete-draft save của UC-01, gồm `baseVersion` check | PASS |
| Environment Configuration Service | Stateless giữa edit request; xử lý load/query/secure Secret staging/save của UC-02, gồm hai concurrency-base checks | PASS |
...
|---|---|---|
| `createApplication()`, `addWorkload()`, `addResourceRequirement()`, `defineConfigurationRequirement()`, `defineDependency()` | `frontend/src/features/application-definition/` — `draft/reducer.ts`, `components/BuilderNavigation.tsx`, `components/OverviewWorkspace.tsx`, `components/WorkloadWorkspace.tsx`, `components/ResourceWorkspace.tsx`, `components/ReviewWorkspace.tsx` | `frontend/src/features/application-definition/draft/reducer.test.ts`; `ApplicationEditorPage.test.tsx` tests focused component workspaces, read-only Review topology and one-request Save |
| Draft in `sessionStorage`: restore after refresh, Discard, clear after Save, discard incompatible data | `frontend/src/features/application-definition/draft/storage.ts`, `frontend/src/features/application-definition/hooks/useApplicationDraft.ts` | `frontend/src/features/application-definition/draft/storage.test.ts`; editor tests for restore, Discard, Save success and Save failure |
...
| `validateApplicationDefinition()` and A1, including naming and uniqueness rules | `backend/internal/domain/appvalidator`; client copy in `frontend/src/features/application-definition/draft/validation.ts` | `appvalidator/validator_test.go`; `frontend/src/features/application-definition/draft/validation.test.ts`; `TestUC01InvalidDraftWritesNothing` (integration) |
| `saveApplicationDefinition()` with `saveNewVersionIfBaseMatches()`, stable IDs and A2 `DRAFT_CONFLICT` | `backend/internal/persistence/application_save.go`; conflict recovery presentation in `frontend/src/features/application-definition/pages/ApplicationEditorPage.tsx` and `frontend/src/features/application-definition/hooks/useDraftExport.ts` | `TestUC01EditCreatesNewVersionWithStableIDs`, `TestUC01StaleBaseVersionIsDraftConflict`, `TestUC01ConcurrentSavesFromSameBase`, `TestUC01RejectsForeignComponentIDAndTakenName` (integration); editor DRAFT_CONFLICT test verifies draft preservation, JSON copy and confirmed reload |
