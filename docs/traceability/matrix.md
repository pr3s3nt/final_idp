...
| UC-02 | Luồng chính: Developer chọn Resource Output / sensitive Resource Output | `bindResourceOutput()` | `docs/use-cases/UC-02/sequence.puml`: Web UI self-message sau catalog query | Web UI | — *(CLIENT-OWNED DTO; output hợp lệ được query từ `resource_definition` qua catalog)* | Không | Không |
...
| UC-02 | Luồng chính/A1: IDP kiểm tra value và output references | `validateEnvironmentConfiguration()` | `docs/use-cases/UC-02/sequence.puml`: Environment Configuration Validator | Environment Configuration Validator | — *(validate configuration draft và catalog results trong memory theo phiên bản mới nhất; output phải thuộc thành phần mà workload depends on)* | Không | Không |
