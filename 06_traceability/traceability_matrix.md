...
| UC-03 | Luồng chính: chọn phiên bản, workload, image/context và tạo deployment để lập plan | `createDeployment()` | `sequence_digrams/uc_03_deploy_application.puml`: Deployment API / Controller → Deployment Orchestrator; Orchestrator canonicalize/hash plan; internal repository message `persistDeployment(...)` ở `AWAITING_CONFIRMATION` | Deployment API / Controller → Deployment Orchestrator | Đọc `application_definition_version` và các bảng theo phiên bản, `environment_configuration`, `resource_definition` (gồm `default_parameters`, `allowed_overrides`, `management_mode`), `resource_instance`, `workload_instance`; ghi `deployment` (gồm `application_definition_version_id`, `plan_fingerprint`, `plan_fingerprint_algo`), `workload_deployment` (`SELECTED`, `wave_number`), `deployment_context` với status `AWAITING_CONFIRMATION`; plan vẫn transient | **Có — Contract 4** | **Có — Deployment:** execution-only Created/Validating → `AWAITING_CONFIRMATION`, hoặc Rejected/A1 |
...
| UC-03 | Luồng chính: dựng dependency/resource graph | `buildDeploymentGraph()` | `sequence_digrams/uc_03_deploy_application.puml`: Deployment Graph Builder (Orchestrator và Deployment Worker cùng gọi) | Deployment Graph Builder | Đọc dữ liệu đã load từ `workload`, `resource_requirement`, `dependency` của phiên bản, `environment_configuration`, `configuration_value`, `secret`; `Deployment Graph` là transient, không có table | Không | **Có — Deployment:** action trước khi vào `AWAITING_CONFIRMATION` |
| UC-03 | Luồng chính: xác định phạm vi, chia tầng, danh sách có thể deploy lại | `planDeploymentWaves()` | `sequence_digrams/uc_03_deploy_application.puml`: Deployment Wave Planner (Orchestrator và Deployment Worker cùng gọi) | Deployment Wave Planner | Đọc `workload_instance` (workload phụ thuộc đang chạy); kết quả transient trong Deployment Graph | Không *(nằm trong Contract 4)* | **Có — Deployment:** action trước khi vào `AWAITING_CONFIRMATION` |
| UC-03 | Luồng chính: resolve Resource Definition theo graph/context | `resolveResourceDefinitions()` | `sequence_digrams/uc_03_deploy_application.puml`: Resource Definition Resolver | Resource Definition Resolver | Đọc `resource_requirement`, `resource_definition` (gồm `management_mode`, `applicability_conditions`) | Không | **Có — Deployment:** action trước khi vào `AWAITING_CONFIRMATION` |
| UC-03 | Luồng chính: xác định create/update/reuse/liên kết infrastructure và thành phần cần gỡ | `planInfrastructureChanges()` | `sequence_digrams/uc_03_deploy_application.puml`: Infrastructure Planner; chạy khi create, khi confirm trước khi tính lại fingerprint, và trong Deployment Worker | Infrastructure Planner | Đọc `resource_instance` theo khóa chủ sở hữu, `workload_instance`; plan là transient, chỉ fingerprint + algorithm được persist trên `deployment` | Không | **Có — Deployment:** action trước `AWAITING_CONFIRMATION` và guard rebuild khi confirm; **Resource Instance:** vào phase Planned (`PLANNED`) cho create/update `MANAGED` |
...
| UC-03 | Luồng chính/A1: Developer xác nhận và chọn **Deploy** | `confirmDeployment()` | `sequence_digrams/uc_03_deploy_application.puml`: Deployment API / Controller → Deployment Orchestrator; load persisted inputs, rebuild plan, recompute/compare fingerprint, validate overrides, rồi internal repository message `confirmDeploymentAndCreateJob(...)` trong một transaction; trả lời ngay | Deployment API / Controller → Deployment Orchestrator | Đọc `deployment`, `workload_deployment`, `deployment_context`, `application_definition_version` và các bảng theo phiên bản, `environment_configuration`, `resource_definition`, `resource_instance`, `workload_instance`; có thể refresh fingerprint khi `PLAN_CHANGED`; atomic CAS `deployment.status = CONFIRMED` và insert `deployment_execution_job` cùng transaction | **Có — Contract 5** | **Có — Deployment:** self-transition `PLAN_CHANGED`/invalid override; `AWAITING_CONFIRMATION` → `CONFIRMED` (có job) chỉ khi fingerprint match và CAS thành công; CAS loser không transition, không job |
| UC-03 | Luồng chính/A2: Deployment Worker reconcile infrastructure của một tầng, hoặc hủy/gỡ liên kết resource không còn trong phiên bản | `reconcileInfrastructure()` | `sequence_digrams/uc_03_deploy_application.puml`: Infrastructure Reconciler (do Deployment Worker gọi) | Infrastructure Reconciler | Ghi/đọc `resource_instance` theo khóa chủ sở hữu; `deployment.status` giữ `DEPLOYING` | **Có — Contract 6** | **Có — Deployment:** self-transition trong `DEPLOYING` hoặc → `FAILED`; **Resource Instance:** `PLANNED` → `PROVISIONING` → `READY`/`FAILED`, reuse tại `READY`, liên kết `EXISTING` → `READY`, `DESTROYED`, `UNLINKED` |
...
| UC-03 | Luồng chính/A2: publish desired deployment state của một tầng sang CD abstraction; lần publish cuối gỡ workload không còn trong phiên bản | `publishDesiredDeploymentState()` | `sequence_digrams/uc_03_deploy_application.puml`: CD Integration / CD Provider Interface (do Deployment Worker gọi); internal repository message `saveWorkloadInstances(...)`/`updateWorkloadInstances(...)` | CD Integration / CD Provider Interface | Ghi `workload_instance` (`DEPLOYING`, `current_workload_deployment_id`; `REMOVED` khi gỡ); `deployment.status` giữ `DEPLOYING`; `delivery_reference` chỉ được persist bởi `saveDeploymentRecord()` | **Có — Contract 8** | **Có — Deployment:** action/failure point trong `DEPLOYING`; **Workload Instance:** → `DEPLOYING`, → `REMOVED` |
...
| UC-03 | Luồng chính: so sánh dấu vân tay output và thêm thành phần phụ thuộc vào các tầng sau | `propagateOutputChanges()` | `sequence_digrams/uc_03_deploy_application.puml`: Deployment Wave Planner (do Deployment Worker gọi); internal repository message `loadOutputFingerprints()`, `loadOutputFingerprintsAndRunningImages()`, `updateOutputFingerprints()`, `updateWorkloadInstances()` | Deployment Wave Planner | Đọc/ghi `resource_instance.output_fingerprint`, `workload_instance.output_fingerprint`; insert `workload_deployment` (`CASCADED`) khi output đổi | **Có — Contract 11** | **Có — Deployment:** self-transition trong `DEPLOYING`; **Resource Instance:** self-transition tại `READY`; **Workload Instance:** → `HEALTHY` |
...
| UC-04 | Luồng chính: chọn một deployment và lấy detail | `getDeploymentDetail()` | `sequence_digrams/uc_04_view_deployment_result.puml`: Deployment Query API / Controller → Deployment Query Service → Deployment Repository | Deployment Query API / Controller → Deployment Query Service → Deployment Repository | Đọc `deployment` (gồm `application_definition_version_id`), `deployment_record`, `deployment_record_resource_instance`; internal message `findWorkloadInstances()` đọc `workload_instance` để biết deployment còn là bản đang chạy hay không | Không | Không — read-only |
...
| Deployment Wave Planner | `planDeploymentWaves()`, `propagateOutputChanges()` | PASS |
...
| Infrastructure Reconciler | `reconcileInfrastructure(finalPlan, planItems)` | PASS |
...
| Provisioner Adapter / Provisioner Interface | Integration abstraction: `reconcile(planItems)` và `destroy(planItems)` delegate provider execution | PASS |
...
| Application Repository | Persistence holder: internal `findLatestVersion()`, `saveNewVersion()`, `listVersions()`, `findVersion()` | PASS |
...
| Resource Instance Repository | `getInfrastructureStatus()` và internal `findResourceInstances(applicationId, environment, resourceRequirementIds, target)`, `saveResourceInstances()`, `loadResourceOutputs()`, `loadOutputFingerprints()`, `updateOutputFingerprints()` | PASS |
...
| `application_definition_version`, `application_component` | W: `saveApplicationDefinition()` (insert phiên bản mới, thành phần mới); R: `updateApplication()`, `loadConfigurationRequirements()`, `createDeployment()`, `confirmDeployment()` | PASS |
...
**Kết luận B4: PASS.** Cả 23 table trong ERD đều có ít nhất một read hoặc write path. `resource_definition` chỉ có read path, được ghi nhận riêng là GAP ở tiêu chí B3; writer của `deployment_step` thuộc D4.
...
**Kết quả tổng thể: PASS có điều kiện.** Design hiện trace được 47/47 Step-2 operations, 46/46 VOPC classes, 23/23 tables theo tiêu chí read-or-write, 11/11 operation contracts và 3/3 state machines. Gap 1, Gap 3 và Gap 4 đã đóng; Gap 2 vẫn cố ý không giải quyết vì writer của `Resource Definition` thuộc platform administration ngoài bốn Developer use case. Kết quả chỉ là coverage/traceability ở mức tài liệu: các mục hoãn D1–D9 (Gap 5), trong đó có writer của `deployment_step` (D4), vẫn mở và phải được giải quyết trước khi coi thiết kế là hoàn chỉnh.
