...
| UC-03 | Luồng chính: chọn image/context và tạo deployment để lập infrastructure plan | `createDeployment()` | `sequence_digrams/uc_03_deploy_application.puml`: Deployment API / Controller → Deployment Orchestrator | Deployment API / Controller → Deployment Orchestrator | Đọc `application_definition`, `workload`, `environment_configuration`, `resource_definition`, `resource_instance`; Contract 4 yêu cầu ghi `deployment`, `workload_deployment`, `deployment_context` nhưng sequence không có repository message tương ứng (**GAP**) | **Có — Contract 4** | **Có — Deployment:** execution-only Created/Validating → `AWAITING_CONFIRMATION`, hoặc Rejected/A1 |
...
| UC-03 | Luồng chính: xác định create/update/reuse infrastructure | `planInfrastructureChanges()` | `sequence_digrams/uc_03_deploy_application.puml`: Infrastructure Planner | Infrastructure Planner | Đọc `resource_instance`; plan là transient, không có table | Không | **Có — Deployment:** action trước `AWAITING_CONFIRMATION`; **Resource Instance:** vào phase Planned (`PLANNED`) cho create/update |
...
| UC-03 | Luồng chính/A1: Developer xác nhận và chọn **Deploy** | `confirmDeployment()` | `sequence_digrams/uc_03_deploy_application.puml`: Deployment API / Controller nhận `confirmDeployment(deploymentId, overrides)`; Deployment Orchestrator tự nhận `confirmDeployment(deploymentId)` sau apply | Deployment API / Controller; Deployment Orchestrator | Contract 5 yêu cầu cập nhật `deployment.status`, nhưng sequence không gọi Deployment Repository (**GAP**) | **Có — Contract 5** | **Có — Deployment:** `AWAITING_CONFIRMATION` → `CONFIRMED`, hoặc self-transition khi override invalid |
| UC-03 | Luồng chính/A2: reconcile infrastructure qua provisioner | `reconcileInfrastructure()` | `sequence_digrams/uc_03_deploy_application.puml`: Infrastructure Reconciler | Infrastructure Reconciler | Ghi/đọc `resource_instance`; Contract 6 còn cập nhật `deployment.status`, nhưng sequence không biểu diễn write này (**GAP**) | **Có — Contract 6** | **Có — Deployment:** `CONFIRMED` → `INFRASTRUCTURE_READY`/`FAILED`; **Resource Instance:** `PLANNED` → `PROVISIONING` → `READY`/`FAILED`, hoặc reuse at `READY` |
...
| UC-03 | Luồng chính/A1/A2: resolve direct value, Resource/Workload Output | `resolveEnvironmentConfiguration()` | `sequence_digrams/uc_03_deploy_application.puml`: Environment Configuration Resolver | Environment Configuration Resolver | Đọc `environment_configuration`, `environment_variable`, `configuration_value`, `secret`, `resource_instance`; Contract 7 cập nhật `deployment.status`, nhưng sequence không biểu diễn write này (**GAP**) | **Có — Contract 7** | **Có — Deployment:** `INFRASTRUCTURE_READY` → `CONFIGURATION_RESOLVED`/`FAILED` |
...
| UC-03 | Luồng chính/A2: publish desired deployment state sang CD abstraction | `publishDesiredDeploymentState()` | `sequence_digrams/uc_03_deploy_application.puml`: CD Integration / CD Provider Interface | CD Integration / CD Provider Interface | Contract 8 cập nhật `deployment.status`; `delivery_reference` chỉ được persist sau bởi `saveDeploymentRecord()`; sequence không biểu diễn status write trực tiếp (**GAP**) | **Có — Contract 8** | **Có — Deployment:** `CONFIGURATION_RESOLVED` → `SUBMITTED`/`FAILED` |
...
| UC-04 | Luồng ngoại lệ A1: hiển thị failed step, component và error summary | `getDeploymentFailureDetail()` | **GAP — không xuất hiện trong `sequence_digrams/uc_04_view_deployment_result.puml`** | **GAP — không class nào trong VOPC khai báo operation này** | Dữ liệu có thật tại `deployment_record.error_summary` và `deployment_step.status`, `related_component_reference`, `error_summary` | Không | Không — operation dự kiến chỉ đọc `FAILED`, không drive state |
...
| UC-02 | 7 | 7 | PASS |
| UC-03 | 18 | 18 | PASS |
| UC-04 | 9 | 8 | **GAP — thiếu `getDeploymentFailureDetail()`** |
| **Tổng** | **43** | **42** | **GAP** |

**Kết luận B1: GAP.** `getDeploymentFailureDetail()` là orphan operation duy nhất: có ở Step 2 nhưng không có message trong bốn sequence diagram.
...
| Deployment Query API / Controller | Nhận `listDeployments()`, `getDeploymentDetail()` | PASS |
...
| Deployment Query Service | Điều phối `listDeployments()`, `getDeploymentDetail()` | PASS |
...
| Environment Configuration Repository | Persistence holder: internal `save()`, `findByApplicationAndEnvironment()` | PASS |
| Resource Instance Repository | `getInfrastructureStatus()` và internal find/save/load output operations | PASS |
| Deployment Repository | `saveDeploymentRecord()`, `getDeploymentDetail()`, `getDeploymentProgress()`, `getDeploymentImages()` và internal `findByApplication()` | PASS |

**Kết luận B2: PASS.** Toàn bộ 42 class hợp nhất trong VOPC có operation cụ thể hoặc có justification rõ ràng là presentation/persistence holder, integration implementation hay external participant. Tuy nhiên, kết quả này không xóa GAP riêng rằng VOPC chưa có owner cho `getDeploymentFailureDetail()`.
...
| Deployment; Workload Deployment; Deployment Context | `deployment`, `workload_deployment`, `deployment_context` | `createDeployment()` theo Contract 4 | PASS về contract/table; **sequence persistence GAP** |
...
**Kết luận B3: GAP.** Tất cả persistent object đều có mapping table, nhưng `Resource Definition` là platform-managed reference data và không có operation create/update/import nào trong Step 2. Vì acceptance criterion yêu cầu ít nhất một writer, không thể coi read-only catalog là PASS. Ngoài ra, writer của Deployment aggregate được Contract 4 quy định nhưng chưa được thể hiện bằng repository message trong sequence.
...
| `deployment`, `workload_deployment`, `deployment_context` | W: `createDeployment()` theo Contract 4; R: các query UC-04 | PASS về operation contract; sequence write chưa tường minh |
| `deployment_record` | W: `saveDeploymentRecord()`; R: `listDeployments()`, `getDeploymentDetail()` | PASS |
| `deployment_step` | W: `saveDeploymentRecord()`; R: `getDeploymentProgress()` | PASS |
...
| 5 | `confirmDeployment()` | PASS — nhưng signature/message split được ghi ở Gaps / Notes |
...
1. **UC-04 orphan operation:** `getDeploymentFailureDetail()` có trong Step 2 nhưng không xuất hiện trong `sequence_digrams/uc_04_view_deployment_result.puml`, đồng thời không được khai báo bởi class nào trong VOPC/design class diagram. Domain/ERD đã có đủ `deployment_record.error_summary` và `deployment_step` để hỗ trợ query này, nhưng trace từ system operation tới interaction/owner đang thiếu.
2. **Thiếu writer cho platform catalog:** `Resource Definition` được phân loại `PERSISTENT (platform-managed)` và có table `resource_definition`, nhưng không có Step-2 operation nào create/update/import dữ liệu catalog. Đây có thể là UC quản trị platform nằm ngoài bốn UC hiện tại, nhưng theo completeness criterion “persistent object phải có writer” thì vẫn là GAP.
3. **UC-03 sequence chưa biểu diễn persistence của Deployment aggregate/status:** Contract 4 yêu cầu `createDeployment()` tạo `deployment`, `workload_deployment`, `deployment_context`; Contracts 5–8 yêu cầu cập nhật `deployment.status`. Tuy nhiên sequence UC-03 chỉ gọi `Deployment Repository.saveDeploymentRecord()` ở cuối và VOPC repository không có operation create/update Deployment tương ứng. Table/contract/state machine nhất quán với nhau, nhưng sequence/VOPC chưa thể hiện write path này.

### Notes

- `confirmDeployment` có ba cách biểu diễn hơi lệch nhau: Step 2 ghi `confirmDeployment()`, boundary message là `confirmDeployment(deploymentId, overrides)`, Contract 5 giữ signature đó, trong khi API gọi `applyInfrastructureOverrides(deploymentId, overrides)` trên Orchestrator rồi Orchestrator tự gọi `confirmDeployment(deploymentId)`. Ý nghĩa nghiệp vụ vẫn trace được, nhưng signature/dispatch chưa hoàn toàn đồng nhất.
...
- Các object `Deployment Graph`, `Resource Resolution`, `Resource Output`, `Resolved Configuration`, `Resolved Specification` là `TRANSIENT`, nên việc không có table tương ứng là chủ đích và không phải GAP.

## Overall acceptance

**Kết quả tổng thể: GAP.** Design có traceability tốt cho 42/43 Step-2 operations, 42/42 VOPC classes, 19/19 tables theo tiêu chí read-or-write, 9/9 operation contracts và 2/2 state machines. Acceptance chưa thể coi là PASS hoàn toàn do ba gap nêu trên: thiếu trace cho `getDeploymentFailureDetail()`, thiếu writer trong scope cho `Resource Definition`, và thiếu repository interaction trong sequence/VOPC để hiện thực các persistent postcondition của Deployment aggregate.
