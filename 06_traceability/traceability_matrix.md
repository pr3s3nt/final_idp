...
Matrix này mô tả coverage sau bản vá 11 review issues trên `draft`, đã được tích hợp làm bản nền phát triển trên `main`. “System operation” là danh sách Bước 2; `executeDeploymentJob`, repository enqueue/step writers và provider staging methods là internal operation và được trace riêng, không cộng giả vào 45 system operations.

**Trạng thái review: OPEN.** Các kết quả `PASS` về coverage bên dưới chỉ xác nhận sự hiện diện/mapping của artifact, không xác nhận thiết kế đã đúng hoặc sẵn sàng code MVP. Review ngày 12/09/2026 còn **10 findings (4 P1, 6 P2)**; xem [báo cáo và danh sách vấn đề còn mở](review_open_issues.md). Cần chốt phạm vi MVP và xử lý các finding liên quan trước khi acceptance phần thiết kế triển khai.
...
**Class coverage: 48/48. Đồng bộ VOPC/sequence: OPEN (R10).** Phân công chính đã thể hiện UI sở hữu draft, Orchestrator accept/enqueue, Worker execute và Query Service read-only. Tuy nhiên VOPC UC-02 còn thiếu repository query operation và VOPC worker còn thiếu dependency tới các component được gọi khi rebuild; chưa thể kết luận các lát cắt đã khớp toàn bộ sequence.
...
- **12/12 contract groups hiện diện:** 1–3 save/generation/configuration, 4–5 prepare/confirm, 6 async worker, 7–11 execution phases/record, 12 UC-04 query guarantees. Behavioral acceptance còn OPEN theo R1–R9.
...
- Resource Instance diagram mô tả plan/reconcile/collect/retire; đường thực thi recovery `FAILED → PLANNED` qua lookup hiện tại còn thiếu (R6).
...
## F. Thay đổi cho 11 review issues ban đầu

Bảng này ghi nhận thay đổi đã có trong bản vá; trạng thái còn mở và điều kiện đóng được theo dõi trong [review_open_issues.md](review_open_issues.md).
...
**COVERAGE VERIFIED — DESIGN REVIEW OPEN.** Bộ tài liệu có 45/45 Step-2 operations, 48 consolidated VOPC classes/interfaces, 21 ERD tables, 12 contract groups và 2 lifecycle state machines. Các số đếm này không đóng 10 findings về confirm, input version, secret promotion, retry/recovery, readiness, step ownership, zero-configuration path và VOPC consistency. Chưa xác nhận thiết kế sẵn sàng triển khai MVP; việc merge vào `main` chỉ thiết lập bản nền để tiếp tục hoàn thiện.

Resource Definition catalog administration và pre-authorization của SHARED_CONSUMER bindings vẫn nằm ngoài bốn Developer use case; đây là ranh giới phạm vi đã nêu rõ, không phải finding còn thiếu writer trong MVP.
