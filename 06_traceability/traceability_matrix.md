...
| UC-03 | Luồng chính: chọn image/context và tạo deployment để lập infrastructure plan | `createDeployment()` | `sequence_digrams/uc_03_deploy_application.puml`: Deployment API / Controller → Deployment Orchestrator; internal repository message là `persistDeployment(..., AWAITING_CONFIRMATION)` | Deployment API / Controller → Deployment Orchestrator | Đọc `application_definition`, `workload`, `environment_configuration`, `resource_definition`, `resource_instance`; ghi `deployment`, `workload_deployment`, `deployment_context` với `deployment.status = AWAITING_CONFIRMATION` | **Có — Contract 4** | **Có — Deployment:** execution-only Created/Validating → `AWAITING_CONFIRMATION`, hoặc Rejected/A1 |
...
| Deployment Repository | `persistDeployment()`, `updateDeploymentStatus()`, `saveDeploymentRecord()`, `getDeploymentDetail()`, `getDeploymentFailureDetail()`, `getDeploymentProgress()`, `getDeploymentImages()` và internal `findByApplication()` | PASS |
...
| Deployment; Workload Deployment; Deployment Context | `deployment`, `workload_deployment`, `deployment_context` | `createDeployment()` qua `persistDeployment(..., AWAITING_CONFIRMATION)`; Contracts 5–8 qua `updateDeploymentStatus()` | PASS |
...
| `deployment`, `workload_deployment`, `deployment_context` | W: `createDeployment()` qua `persistDeployment()` và `updateDeploymentStatus()`; R: các query UC-04 | PASS |
