...
- **Operation**: `createDeployment(applicationId, version, environment, target, images, context)`
...
- **Preconditions**:
  ...
  - Nếu chỉ một phần Workload của phiên bản được chọn, `version` trùng phiên bản đang chạy trên `(environment, target)` (theo `workload_instance.current_workload_deployment_id` → `deployment.application_definition_version_id`). Đổi sang phiên bản khác phải chọn toàn bộ Workload.
  ...
  - Với mỗi `Resource Requirement` trong phạm vi, Platform Resource Definition Catalog có một `Resource Definition` phù hợp với `resourceType`, `supportedContexts` và `applicability_conditions` (nếu có), cùng `provisionerReference` hợp lệ, đồng thời cung cấp `default_parameters` và `allowed_overrides`.
- **Postconditions**:
  - Một instance `Deployment` được tạo; một row `deployment` được tạo với `deployment_id`, `application_id`, `application_definition_version_id`, `environment_configuration_id`, `environment`, `deployment_target`, `plan_fingerprint`, `plan_fingerprint_algo`, `created_at`, `updated_at`, và `status = AWAITING_CONFIRMATION`.
  - Association `Deployment` → `Application Definition Version` và `Deployment` → `Environment Configuration` được hình thành qua `application_definition_version_id` và `environment_configuration_id`; environment của configuration, snapshot `deployment.environment` và input `environment` bằng nhau.
  ...
  - Một instance `Deployment Graph` `TRANSIENT` được tạo cho execution hiện tại với `deploymentId`, `workloadIds`, `resourceRequirementIds`, `dependencyIds`, `configurationReferenceIds`, và kết quả `planDeploymentWaves()`: `scopeComponentIds` (Workload được chọn và Resource Requirement mà chúng depends on trực tiếp), `waves` theo thứ tự phụ thuộc, `potentialRedeployComponentIds` (thành phần ngoài phạm vi depends on thành phần trong phạm vi và đang chạy).
  - Với mỗi Resource Requirement trong phạm vi, một instance `Resource Resolution` `TRANSIENT` được tạo và liên kết tới đúng `Resource Definition` (`MANAGED` hoặc `EXISTING`). Resource Instance hiện có được tìm theo đúng khóa chủ sở hữu `(application_id, environment, resource_requirement_id, deployment_target)`.
  - Execution state chứa infrastructure plan ở mức khái niệm (cấu trúc chi tiết để D3): theo từng tầng, mỗi resource là create, update, reuse hoặc liên kết resource `EXISTING`; khi `version` khác phiên bản đang chạy, thêm Workload cần gỡ, resource `MANAGED` cần hủy và resource `EXISTING` cần gỡ liên kết; cùng danh sách override được phép.
  - Infrastructure plan được canonicalize bằng stable resource/key ordering và canonical JSON, normalize unit/number, đồng thời loại timestamps và auto-generated ID không mang ý nghĩa nghiệp vụ. Fingerprint domain bao phủ action `CREATE`/`UPDATE`/`REUSE` của từng resource; identity của Resource Definition được tham chiếu và các field ảnh hưởng plan gồm `provisioner_reference`, `supported_contexts`, `default_parameters` và `allowed_overrides`; resolved parameters của plan được suy ra một cách tất định từ `default_parameters` kết hợp deployment context, còn allowed-overrides definition (tập key được phép cùng tập giá trị, min-max hoặc enum) được suy ra một cách tất định từ `allowed_overrides`; Resource Instance identity được tham chiếu; và deployment target. Mọi thay đổi đối với bất kỳ field Resource Definition nào trong bốn field trên làm fingerprint thay đổi trực tiếp; không cần cột version trên `resource_definition`.
  ...
  - Chưa có `Resource Instance` hay `Workload Instance` nào bị tạo, sửa hoặc gỡ bởi operation này; chưa có `Deployment Execution Job`, `Deployment Record`, `Deployment Step`, desired deployment state hay CD delivery reference nào được tạo.
- **Exceptions / Guarantees**:
  - A1: nếu phiên bản không tồn tại, image version, required configuration, cấu hình không khớp phiên bản, deploy một phần với phiên bản khác phiên bản đang chạy, dependency tạo vòng hoặc không resolve được, Workload phụ thuộc ngoài phạm vi chưa chạy healthy, output reference, Resource Definition hoặc context không hợp lệ, không aggregate `Deployment` hoàn chỉnh nào tồn tại; các row `deployment`, `workload_deployment`, `deployment_context` phát sinh trong attempt được rollback và infrastructure không thay đổi.
...
- **Preconditions**:
  ...
  - Các input bền vững để rebuild plan còn tồn tại và nhất quán: Application Definition Version được tham chiếu, Environment Configuration, snapshot Workload Deployment/images, Deployment Context và deployment target; Resource Definition Catalog có `default_parameters`/`allowed_overrides` cho mỗi definition được match, cùng current Resource Instance và Workload Instance state, có thể được đọc lại.
...
- **Postconditions**:
  ...
  - Association của Deployment với `Application Definition Version`, `Environment Configuration`, `Workload Deployment` và `Deployment Context` không đổi; phiên bản, image version, environment và deployment target đã snapshot không đổi.
- **Exceptions / Guarantees**:
...
- **Operation**: `reconcileInfrastructure(finalPlan, planItems)`
- **Cross References**: UC-03 – Deploy Application, bước Deployment Worker triển khai resource của một tầng, và bước xử lý thành phần không còn trong phiên bản; A2 – Provisioning, delivery hoặc workload thất bại.
- **Preconditions**:
  - Deployment đang ở `status = DEPLOYING` và được Deployment Worker thực thi từ job đã lưu. `finalPlan` là plan dựng lại từ persisted inputs với `selected_overrides` của job; `planItems` là các resource item của **một tầng**, hoặc các item gỡ/hủy/gỡ liên kết khi phiên bản được deploy không còn resource đó.
  - Mỗi item đã resolve tới một `Resource Definition` có `resourceDefinitionId`, `provisionerReference` (với `MANAGED`) hoặc `existing_resource_reference` (với `EXISTING`), và `supportedContexts` phù hợp với `Deployment Context`/`deploymentTarget`.
  - Resource Instance mục tiêu được xác định theo khóa chủ sở hữu `(application_id, environment, resource_requirement_id, deployment_target)`: với item update/reuse/hủy/gỡ liên kết, instance đó tồn tại; với item create hoặc liên kết lần đầu, chưa có instance đang dùng cho khóa này.
  - Mọi resource mà các item phụ thuộc đã sẵn sàng ở các tầng trước; Provisioner Adapter và Terraform/OpenTofu Runner tương ứng sẵn sàng với item `MANAGED`.
- **Postconditions**:
  - Với mỗi item create (`MANAGED`) thành công, một instance `Resource Instance` và một row `resource_instance` được tạo với `application_id`, `environment`, `resource_requirement_id`, `resource_definition_id`, `deployment_target`, `infrastructure_reference`, `provider_state_reference`, `created_at`, `updated_at` và `status = READY`.
  - Với mỗi item update (`MANAGED`) thành công, `infrastructure_reference`, `provider_state_reference`, `status = READY`, `updated_at` được cập nhật theo provider state thực tế; identity và `created_at` không đổi.
  - Với mỗi item reuse, instance hiện hữu được giữ; identity và `infrastructure_reference` không đổi, `status` được xác nhận là `READY` và `updated_at` chỉ thay đổi nếu trạng thái provider được refresh.
  - Với mỗi item liên kết resource `EXISTING`, không provisioner nào được gọi và hạ tầng thật không thay đổi; instance của chủ sở hữu được tạo hoặc giữ với `infrastructure_reference = existing_resource_reference` và `status = READY`.
...
- **Cross References**: UC-03 – Deploy Application, bước sau mỗi tầng Deployment Worker so sánh dấu vân tay output và tự động thêm thành phần phụ thuộc vào các tầng sau.
- **Preconditions**:
  - Deployment đang ở `status = DEPLOYING`; mọi thành phần trong `waveComponents` đã sẵn sàng (resource `READY`, workload healthy) và `collectedOutputs` chứa output của chúng.
...
- **Exceptions / Guarantees**:
  ...
  - Lan truyền dừng khi không còn dấu vân tay nào thay đổi; mỗi workload được thêm tối đa một lần trong một deployment.
